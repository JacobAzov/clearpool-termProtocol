...
contract TermPool is ITermPool, Initializable, TermUtils, ReentrancyGuardUpgradeable {
  ...
  function createTerm(
    ...
    // Concatenate the prefix, and the encoded suffix while by taking the first five characters from the encoded suffix. ex: App + 3f412
    vars.tpSymbol = bytes.concat(vars.prefix, store5Chars(vars.encoded));

    TpToken(tpTokenAddr).__TpToken_init(
      string(bytes.concat(bytes('Term Pool '), bytes(vars.prefixSymbol), bytes('-'), vars.suffix)),
